...
if (!function_exists('settings')) {
    ...
    {
        return app(App\Services\SettingService::class)->module($module);
    }
}
...
if (!function_exists('getRouteAction')) {
     ...
     */
    function getRouteAction(string $routeName): string
    {
        $action = Illuminate\Support\Str::afterLast($routeName, '.');

        return match ($action) {
            'index', 'create', 'update', 'store', 'edit' => $action,
            default => Illuminate\Support\Arr::last(
