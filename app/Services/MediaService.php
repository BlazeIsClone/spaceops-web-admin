...
class MediaService
{
	 ...
	 */
	public function restore(string $fileId): ?array
	{
		...
		return [
			'mime_type' => Storage::mimeType($this::DISK . DIRECTORY_SEPARATOR . $filePath),
			'file_name' => $this->getTempFileName($fileId),
			'file' => $this->disk()->get($filePath),
		];
	}

	/**
	 * Revert temporary uploaded file.
	 */
	public function revert(Request $request): bool
	{
		...
		$this->deleteTempFileDirectory($fileId);
	 ...
	 */
	protected function getTempFileName(string $fileId): string
	{
		return basename($this::DISK . $this->getTempFilePath($fileId));
	}
