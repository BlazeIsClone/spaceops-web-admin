...
class AdminBaseController extends Controller implements AdminBaseControllerInterface
{
	...
	/**
	 * Get the route prefix name.
	 */
	public function getRoutePrefix(): string
	{
		$modelName = $this->getService()->modelName();

		return Str::of($modelName)->plural()->kebab();
	}
	 ...
	 */
	private function getRouteAction(string $routeName = null): string
	{
		$action = Str::afterLast(
			!$routeName ? Route::currentRouteName() : $routeName,
			'.'
		);
	 ...
	 */
	public function tableColumns(array $columns, array $prepends = ['id'], array $appends = ['status', 'actions'])
	{
		return array_merge($prepends, $columns, $appends);
	}
